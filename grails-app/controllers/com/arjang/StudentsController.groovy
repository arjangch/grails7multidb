...
class StudentsController {
    ...
    def withAnnotation(){
        log.info '-1--='+ helpersService.withAnnotation()
        redirect(uri: "/")
    }

    def usingTransaction() {
        log.info '-2--='+ helpersService.usingTransaction()
        redirect(uri: "/")
    }

    def createNewSession() {
        log.info '-2--='+ helpersService.createNewSession()
        redirect(uri: "/")
    }

    def usingSession() {
        log.info '-2--='+ helpersService.usingSession()
        redirect(uri: "/")
    }

    def justCallObject() {
        log.info '-2--='+ helpersService.justCallObject()
        redirect(uri: "/")
    }
